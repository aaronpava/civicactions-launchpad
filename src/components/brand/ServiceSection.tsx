...
function Check() {
  return (
    <svg
      ...
      aria-hidden
      className="mt-1.5 size-4 shrink-0 text-red"
    >
  ...
  return (
    <section id={service.id} aria-labelledby={headingId} className="scroll-mt-32 py-12 first:pt-4">
      <div className="flex items-center gap-3">
        <HomeServiceIcon icon={service.icon} className="size-6 shrink-0 text-red" />
        <p className="eyebrow text-red">{service.eyebrow}</p>
      </div>
      ...
      <ul className="mt-4 space-y-5">
        {service.helped.map((item) => (
          <li key={`${item.client}-${item.result}`}>
            ...
            {item.href ? (
              <Link to={item.href} className="text-red underline">
                {item.result}
              </Link>
            ) : (
              <p className="text-red underline">{item.result}</p>
            )}
